...
/**
 * 아래 네 필드는 테스트별로 직접 쓴 글입니다. 채워진 테스트만 해당 섹션이 나오므로
 * 27개를 한꺼번에 채울 필요는 없습니다. 유형별 상세 가이드와 추천 테스트는
 * results·related 데이터로 자동 생성되어 모든 테스트에 이미 붙습니다.
 */
export type GenericTestCopy = {
  /** 페이지 H1. 없으면 title 을 씁니다. 카탈로그 카드 이름과 분리하기 위한 필드 */
  heading?: string;
  /** 개념 섹션 제목. 없으면 "OO 알아보기" 로 만듭니다 */
  introHeading?: string;
  /** 개념 정의 문단. "OO란?" 검색 유입용 */
  intro?: string[];
  /** 이 테스트로 알 수 있는 것 (3줄 내외) */
  whatYouLearn?: string[];
  /** 유형 조합별 궁합표 */
  matchTable?: Array<{ pair: string; note: string }>;
  /** 자주 묻는 질문. 화면과 FAQPage 스키마가 같은 문구를 씁니다 */
  faq?: Array<{ q: string; a: string }>;
};

export type GenericTest = GenericTestCopy & {
  slug: string;
  title: string;
  eyebrow: string;
  description: string;
...
export const genericTests: Record<string, GenericTest> = {
  "egen-teto": {
    ...
    heading: "나는 에겐일까, 테토일까? 무료 20문항 에너지 유형 테스트",
    introHeading: "에겐테토란?",
    intro: [
      "에겐테토는 에스트로겐(Estrogen)의 '에겐'과 테스토스테론(Testosterone)의 '테토'에서 따온 신조어로, 사람의 성향을 감정형(에겐)과 추진형(테토)이라는 두 가지 에너지로 나눠보는 성격 유형 테스트입니다. 공감과 관계를 중시하는 부드러운 에너지가 '에겐', 추진력과 행동력이 두드러지는 강한 에너지가 '테토'입니다.",
      "정식 심리학 이론은 아니지만 SNS와 유튜브 쇼츠를 중심으로 퍼지면서 MBTI만큼 익숙한 표현이 되었습니다. 호르몬 수치를 재는 검사가 아니라 대중문화 속 이미지를 빌린 성향 테스트라는 점을 기억하고 가볍게 즐겨주세요.",
    ],
    whatYouLearn: [
      "나의 대인관계 스타일이 공감형에 가까운지 주도형에 가까운지",
      "연애와 친구 관계에서 내가 에너지를 쓰는 방식",
      "나와 반대이거나 비슷한 성향인 사람과의 궁합 포인트",
    ],
    matchTable: [
      { pair: "에겐녀 × 테토남", note: "가장 상호보완적인 조합입니다. 부드러움과 든든함이 균형을 이룹니다." },
      { pair: "에겐남 × 테토녀", note: "다정한 표현이 직진하는 에너지를 편안하게 받쳐줍니다." },
      { pair: "에겐 × 에겐", note: "공감은 넘치지만 결정을 미루다 답답해질 수 있습니다. 한 명이 방향을 정해주면 좋습니다." },
      { pair: "테토 × 테토", note: "서로 존중하면 강력한 팀이지만, 물러서지 않으면 자존심 싸움이 되기 쉽습니다." },
      { pair: "균형형 × 누구든", note: "상황에 따라 두 에너지를 모두 쓰기 때문에 조율자 역할을 맡기 좋습니다." },
    ],
    faq: [
      {
        q: "에겐테토 테스트는 MBTI랑 뭐가 달라요?",
        a: "MBTI가 4가지 지표로 16가지 성격을 나누는 정교한 심리 모델이라면, 에겐테토는 관계 안에서의 에너지 방향만 보는 훨씬 단순하고 직관적인 테스트입니다. 재미로 가볍게 즐기기 좋고, MBTI와 함께 보면 나를 더 입체적으로 이해할 수 있습니다.",
      },
      {
        q: "결과가 애매하게 나왔어요, 왜 그런가요?",
        a: "에겐과 테토 성향을 상황에 따라 다르게 쓰는 분들도 많습니다. 친한 사람 앞에서는 에겐, 낯선 자리에서는 테토처럼요. 그런 경우에는 균형형으로 나오며, 두 성향의 장점을 모두 갖고 있다고 이해하시면 됩니다.",
      },
      {
        q: "에겐테토 테스트는 과학적으로 검증된 건가요?",
        a: "아니요. 정식 심리학 이론이 아니라 SNS에서 시작된 대중문화 콘텐츠입니다. 실제 호르몬 수치를 측정하지도 않습니다. 정확한 심리 분석보다는 나를 가볍게 돌아보는 놀이로 즐겨주세요. 더 정교한 성격 분석이 궁금하다면 MBTI 검사를 함께 해보시는 것을 추천합니다.",
      },
    ],
    results: {
