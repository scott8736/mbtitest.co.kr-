 ...
 * 문항과 결과를 만드는 재료.
 *
 * 이름·설명·카테고리 같은 표시용 값은 lib/test-meta.ts 에 있습니다. 여기 두면
 * 카탈로그를 읽는 쪽이 이 파일 전체를 함께 내려받게 됩니다.
 */
type TestSeed = {
  slug: string;
...
const makeTest = (seed: TestSeed): GenericTest => {
  ...
  return {
    ...
    related: relatedByCategory[meta.category].filter((slug) => slug !== seed.slug).slice(0, 3),
  };
};
...
const seeds: TestSeed[] = [
  ...
  { slug: "vegetable-village-character",
    results:[
  ...
  { slug: "family-letter-personality",
    results:[
  ...
  { slug: "christmas-cookie-personality",
    results:[
